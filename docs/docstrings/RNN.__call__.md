...
Info
----
Input is `(batch, time, features)` with one leading batch dimension. Pass `hx` to override the default zero initial state, for example when continuing a sequence across chunks.
