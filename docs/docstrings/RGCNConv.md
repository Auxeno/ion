...
\[
x'_i = W_0 x_i + \sum_{r} \frac{1}{|\mathcal N_r(i)|}
...
----------
w_neigh : Param
    Neighbour transforms of shape `(num_bases or num_edge_types, in_dim, out_dim)`.
w_coeff : Param | None
    Mixing coefficients of shape `(num_edge_types, num_bases)`. `None` when
...
num_nodes, num_edge_types, in_dim, out_dim = 3, 2, 16, 32
x = jnp.ones((num_nodes, in_dim))
senders = jnp.array([0, 1, 2])
receivers = jnp.array([1, 2, 0])
edge_type = jnp.array([0, 1, 0])

conv = gnn.RGCNConv(in_dim, out_dim, num_edge_types, key=key)
y = conv(x, senders, receivers, edge_type=edge_type)  # (3, 16) -> (3, 32)
```

```python
conv = gnn.RGCNConv(in_dim, out_dim, num_edge_types, num_bases=2, key=key)
```

Note
----
The forward pass projects every node under every relation before gathering the
one each edge needs, so it holds a `(num_nodes, num_edge_types, out_dim)`
