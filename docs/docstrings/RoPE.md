...
The sequence axis defaults to `-2` and can be selected at call time for other
layouts, such as projected attention tensors shaped `(..., sequence, heads, head_dim)`.
...
num_prefix_tokens : int, default=0
    ...
    lattice and should stay position-invariant.
theta : float, default=10000.0
    Base wavelength controlling the rotation frequencies across feature pairs.

Notes
-----
`shape` and `num_prefix_tokens` are static configuration, so a RoPE is fixed to one
lattice. That is what lets it be bound into an `MultiHeadAttention` layer's `attention_fn` at
construction time.
