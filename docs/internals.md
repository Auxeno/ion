...
- **Frozen params still occupy optimizer state and gradient memory.** `stop_gradient` makes XLA skip backward computation, but `jax.grad` still returns zero arrays at frozen positions and `optimizer.init(model)` allocates momentum/variance buffers for every leaf. For large models where most params are frozen (e.g. LoRA fine-tuning), this can waste significant memory. To avoid this, partition the model so only trainable params are passed as the differentiable argument to the loss function and to `optimizer.init`.
