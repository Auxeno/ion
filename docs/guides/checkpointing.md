...
Non-array fields (ints, strings, activation functions) come from the reference tree, not the file. If you change an activation in code and load an old checkpoint, you get the new activation with the old weights, with no warning. See [Sharp Edges](sharp-edges.md).
