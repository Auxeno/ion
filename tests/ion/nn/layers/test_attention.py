...
class TestSelfAttention:
    ...
    def test_mask_with_causal(self):
        ...
        mask = jnp.ones((4, 4), dtype=bool).at[1, 0].set(False)
        jac = jax.jacobian(lambda x: layer(x, mask=mask))(x)
        jac_seq = jnp.sum(jnp.abs(jac), axis=(1, 3))
    ...
    def test_mask_per_head(self):
        ...
        x = jax.random.normal(jax.random.key(1), (4, 8))
        mask = jnp.ones((1, 4, 4), dtype=bool).at[0, 0, 1].set(False)
        y = layer(x, mask=mask)
        assert y.shape == (4, 8)
...
class TestCrossAttention:
    ...
    def test_mask_blocks_context_positions(self):
        ...
        ctx = jax.random.normal(jax.random.key(2), (5, 8))
        # Block query position 0 from attending to context position 2
        mask = jnp.ones((3, 5), dtype=bool).at[0, 2].set(False)
        ...
        npt.assert_allclose(jac[0, :, 2, :], 0.0, atol=1e-5)
    ...
    def test_mask_per_head(self):
        ...
        x = jax.random.normal(jax.random.key(1), (3, 8))
        ctx = jax.random.normal(jax.random.key(2), (5, 8))
        mask = jnp.ones((1, 3, 5), dtype=bool).at[0, 0, 0].set(False)
